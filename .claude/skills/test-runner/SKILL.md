...
## 基线（用前以实跑为准）
- 全量：**1702** 测试 = unit **777** + integration **842** + e2e **83**（每个用例恰属一层，无重复/无遗漏）。
- 已知 **6 个"待定·不改产品"失败**（非回归，勿当新 bug）：`4 failures + 2 errors`；另 `14 skipped` 为合理环境跳过。
  - 分布：unit 层 `1 failure + 2 errors`，integration 层 `3 failures`，e2e 层全过。
- 详见 memory `project_test_suite_state.md`。
