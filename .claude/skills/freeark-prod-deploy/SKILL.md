...
> **信息快照：2026-05-31（perf-P1a：Redis Channel Layer + --workers 2；前快照 2026-05-29 v0.6.3）。** 带 ⚠️ 的项为易变 / 用前需实测复核。
...
| 服务 | 作用 | 入口 |
|---|---|---|
| `redis-server` | **Redis（Channel Layer，127.0.0.1:6379）**（perf-P1a 新增，apt 安装，开机自启）| apt 系统服务 |
...
`settings.py` 必含：`'channels'` 在 INSTALLED_APPS；`CHANNEL_LAYERS` 生产用 `channels_redis.core.RedisChannelLayer`（hosts `127.0.0.1:6379`）；测试分支（`_RUNNING_TESTS`）仍用 `InMemoryChannelLayer`（不依赖 Redis 服务）。
...
本手册信息采集于 **2026-05-23（OpenClaw 集成 + ASGI 迁移完成日）**，**2026-05-29 补 `freeark-fault-consumer` 与 `freeark-fault-cleanup.timer`**（v0.6.3 部署期发现遗漏），**2026-05-31 perf-P1a（Redis Channel Layer + `--workers 2`）**。基础设施变更后请同步更新本文件。
