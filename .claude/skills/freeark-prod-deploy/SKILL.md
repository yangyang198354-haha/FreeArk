...
> **信息快照：2026-05-31（perf-P2：Redis 缓存后端（db=1）+ redis-py 5.x 固定；前快照 perf-P1a：Redis Channel Layer + --workers 2）。** 带 ⚠️ 的项为易变 / 用前需实测复核。
...
| 服务 | 作用 | 入口 |
|---|---|---|
| `redis-server` | **Redis（Channel Layer db=0 + 缓存后端 db=1，127.0.0.1:6379）**（perf-P1a 新增，apt 安装，开机自启）| apt 系统服务 |
| `freeark-backend` | **Django Web (Uvicorn ASGI, :8000)** | `uvicorn freearkweb.asgi:application --app-dir FreeArkWeb/backend/freearkweb --workers 2`（perf-P1a：1→2）|
...
- Uvicorn 启动参数：`--host 0.0.0.0 --port 8000 --workers 2 --app-dir /home/yangyang/Freeark/FreeArk/FreeArkWeb/backend/freearkweb`
  - **perf-P1a（2026-05-31）**：`--workers 1` 改为 `--workers 2`（换用 RedisChannelLayer，Pi 5 4核，2 worker 约 200MB）
  - **回滚**：改回 `--workers 1` + settings.py CHANNEL_LAYERS 改回 InMemoryChannelLayer
...
`settings.py` 必含：`'channels'` 在 INSTALLED_APPS；`CHANNEL_LAYERS` 生产用 `channels_redis.core.RedisChannelLayer`（hosts `127.0.0.1:6379`，db=0）；测试分支（`_RUNNING_TESTS`）仍用 `InMemoryChannelLayer`（不依赖 Redis 服务）。
...
**perf-P1a 新增：多 worker 退化行为（功能均正确，仅做记录）**
- **看板 LocMemCache**：多 worker 下各 worker 各自维护进程内缓存（per-worker），命中率稳态与单 worker 相同，TTL=30s 内数据轻微不一致属设计接受范围。
- **`_activity_cache` 节流字典**（api/authentication.py）：多 worker 下每 worker 各自维护，同一 token 的节流 DB UPDATE 最多变为 N-worker 倍（2 worker = 最多 2×/5min），**超时判定仍以 DB 的 TokenActivity.last_active_at 为权威，会话超时（v0.9.0）行为完全正确**。
...
**perf-P1a 新增坑（Redis Channel Layer + 多 worker，2026-05-31）：**

- **Redis 未启动 → backend 启动失败**：`channels_redis` 在 Django 启动时不立即连接 Redis（惰性连接），但第一个 WebSocket 请求到来时若 Redis 不可用会抛 `ConnectionRefusedError`。排查：`systemctl is-active redis-server`；`redis-cli ping` 应返 `PONG`。
- **redis-server bind 配置**：默认 `/etc/redis/redis.conf` 的 `bind 127.0.0.1 -::1`，确认无 `0.0.0.0`（安全要求：Redis 只绑 loopback，不对外暴露）。
- **`freeark-backend.service` 的 `After=redis-server.service`**：确保 Redis 在 backend 前启动。若 Redis 未加入 After，Redis 延迟启动时 backend 首次 WS 请求可能报错（会自动重试恢复，但日志会有 ERROR）。
- **hiredis 在 ARM64 上**：channels_redis 会尝试 `import hiredis`，若不可用自动降级纯 Python redis 客户端（无副作用，性能略降但 Channel Layer 消息量极小，可忽略）。若安装时出现 hiredis 编译报错，`pip install channels_redis --no-deps && pip install redis` 绕过即可。
- **多 worker 下 WebSocket 连接**：每个 WebSocket 连接由握手时接收它的 worker 独占，`uvicorn --workers 2` 的负载均衡由 OS 在 accept() 层完成（round-robin / least-conn 取决于 OS）。长连接不会在 worker 间迁移，不存在消息丢失风险。
- **回滚路径**：`settings.py` CHANNEL_LAYERS 改回 `InMemoryChannelLayer` + systemd unit `--workers 1` + `daemon-reload` + `restart freeark-backend`，5 分钟内完成，无需停 Redis。
...
本手册信息采集于 **2026-05-23（OpenClaw 集成 + ASGI 迁移完成日）**，**2026-05-29 补 `freeark-fault-consumer` 与 `freeark-fault-cleanup.timer`**（v0.6.3 部署期发现遗漏），**2026-05-31 perf-P1a（Redis Channel Layer + `--workers 2`）**，**2026-05-31 perf-P2（看板接口 Redis 缓存后端 db=1 + redis-py 5.x 固定）**。基础设施变更后请同步更新本文件。
