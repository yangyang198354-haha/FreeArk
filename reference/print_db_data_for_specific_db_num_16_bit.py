...
class S7PLCReader:
    ...
    @staticmethod
    def parse_uint16(raw_data: bytes, offset: int) -> Optional[int]:
        """解析16位无符号数（大端模式），带异常提示"""
        if len(raw_data) != DATA_LENGTH:
            print(f"❌ 偏移量{offset}：解析失败（数据长度不足，需{DATA_LENGTH}字节）")
            return None

        try:
            # 西门子PLC默认大端字节序（>），H表示16位无符号整数
            return struct.unpack('>H', raw_data)[0]
        except Exception as e:
            print(f"❌ 偏移量{offset}：解析异常 - {str(e)}（数据格式非16位无符号数）")
            return None

    def read_uint16_range(self, db_num: int, start: int, end: int) -> None:
        """读取从start到end的偏移量（逐1递增），解析16位无符号数"""
        for offset in range(start, end + 1):
            ...
            # 解析并打印结果
            value = self.parse_uint16(raw_data, offset)
...
if __name__ == "__main__":
    ...
    try:
        ...
        # 循环读取偏移量0~999的16位无符号数
        while True:
            print("\n" + "=" * 70)
            print(f"📅 本轮读取开始时间：{time.strftime('%Y-%m-%d %H:%M:%S')}")
            print(f"📌 读取目标：DB{DB_NUMBER}（偏移量{START_OFFSET}至{END_OFFSET}，16位无符号数）")
            print("-" * 70)

            # 按偏移量逐1递增读取并打印
            plc_reader.read_uint16_range(DB_NUMBER, START_OFFSET, END_OFFSET)
