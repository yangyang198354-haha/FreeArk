...
# MOD-DC-01: 大屏连通性 ICMP 探测任务已停用。
# 新方案改为由 freeark-screen-heartbeat.service 独立订阅 MQTT 心跳实现。
...
class TaskScheduler:
    ...
    def start(self):
        ...
        # MOD-DC-01: ICMP 探测线程已停用，改由 freeark-screen-heartbeat.service 处理。
        # self._start_screen_connectivity_task()

    def _start_screen_connectivity_task(self):
        """MOD-DC-01: 已停用。

        大屏连通性探测已由独立的 freeark-screen-heartbeat.service 通过 MQTT 心跳实现，
        不再在 task_scheduler 内启动 ICMP 探测线程。
        保留空方法以兼容可能存在的外部调用。
        """
        logger.info(
            'ScreenConnectivity: ICMP 探测线程已停用，'
            '请确认 freeark-screen-heartbeat.service 已在服务器上运行。'
        )
    ...
    def stop(self):
        ...
        # MOD-DC-01: 大屏探测线程已停用，freeark-screen-heartbeat.service 独立管理。
