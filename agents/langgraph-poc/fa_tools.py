"""
langgraph-poc — FreeArk 工具桥接层（进程内复用现有 freeark-skill handlers）

核心论点：当前 OpenClaw 链路里，每次工具调用都是 LLM 经 Bash 工具 `exec python3
freeark_tool.py`——一次子进程冷启动 + urllib 新连接。本桥接层把同一批
TIER1_HANDLERS / TIER2_HANDLERS **直接 import 进编排进程**，包成 LangChain @tool：

  - 零子进程：handler 就是普通 Python 函数，LangGraph 节点直接调用
  - 连接可复用：FreeArkClient 仍是 urllib，但同进程内可换 httpx 连接池（见 README §5）
  - 工具语义不变：参数/返回与生产 skill 完全一致，迁移零认知负担

离线模式（FREEARK_POC_MOCK=1）：handler 需要 FREEARK_AGENT_TOKEN + 127.0.0.1:8000，
PoC 结构验证时无后端，故 mock 包装返回最小 canned 数据，保持工具表一致可跑。

文档引用：detailed_design.md（多 agent 框架）, lobster-agent-architecture（[[lobster-agent-architecture]]）
"""
...
# ── 把 freeark-skill 的 scripts/lib 挂上 sys.path，直接复用生产 handler ──
# 路径解析优先级：FREEARK_SKILL_DIR 环境变量 > 相对本文件的 ../freeark-skill。
# 前者让 PoC 能从任意目录（如 Pi 上的 /tmp/lg-poc）指向真实 skill 而无需搬动代码。
_POC_DIR = Path(__file__).resolve().parent
_SKILL_DIR = Path(os.environ.get("FREEARK_SKILL_DIR") or (_POC_DIR.parent / "freeark-skill"))
_SCRIPTS = _SKILL_DIR / "scripts"
_LIB = _SKILL_DIR / "lib"
for p in (_SCRIPTS, _LIB):
...
try:
    ...
    if not _MOCK:
        raise RuntimeError(
            f"无法导入 freeark-skill handlers: {exc}. "
            f"离线结构验证请设 FREEARK_POC_MOCK=1"
        ) from exc
...
# ── 只读冒烟自检：`python fa_tools.py --smoke` ──────────────────────────
