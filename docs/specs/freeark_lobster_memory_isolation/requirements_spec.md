...
本文档是 **增量需求规格**，描述 FreeArk 方舟智能体多用户记忆隔离功能的需求。
...
本期核心目标：确保不同 FreeArk 用户与方舟智能体的对话记忆相互隔离，智能体的核心人格/工具集不被用户对话修改，每个用户拥有私有的对话历史记忆。
...
> "不同 FreeArk user 和方舟智能体聊天的时候要求方舟智能体的记忆不会漂移。每个 FreeArk user 和智能体的记忆单独保存。智能体的人格、身份等不会被 user 修改。但是智能体有和这个 user 对话的历史记忆。"
...
| FACT-10 | 方舟智能体的人格骨架文件（SOUL.md、AGENTS.md、TOOLS.md、USER.md）在生产上没有任何写保护机制，LLM 在工具调用中理论上可读写它们 | 生产环境结构观察 |
...
**子问题 P-1：记忆漂移**
当前 OpenClaw workspace/memory/ 目录全局共享。如果 OpenClaw 将对话结果写入记忆文件，用户 A 的对话内容会影响用户 B 看到的智能体记忆。

**子问题 P-2：人格漂移**
如果用户通过对话诱导 LLM 修改 SOUL.md / AGENTS.md 等骨架文件，修改结果会对所有用户永久生效，且无法追溯。

**子问题 P-3：历史记忆缺失**
当前每次 WS 连接生成新的 session_key，FreeArk 侧无跨连接历史，用户每次打开聊天都是"全新"的智能体，没有"上次聊过什么"的记忆。
...
**描述**：系统应为每个 FreeArk 用户维护与方舟智能体的历史对话摘要或完整记录，使智能体能在不同 WS 连接之间"记住"曾与该用户聊过的内容。

**来源**：用户原始需求（"智能体有和这个 user 对话的历史记忆"）；FACT-07、FACT-08（当前历史无法跨连接持久化）
...
- **AC-013-01**（历史记忆跨连接可见）
  - Given：FreeArk 用户 A 在第 1 次 WS 连接中与智能体讨论了"我喜欢喝咖啡"
...
- **AC-013-02**（历史记忆用户隔离）
  - Given：用户 A 的历史记忆已存储
  - When：用户 B 登录并与智能体对话
  - Then：智能体不会将用户 A 的历史内容呈现给用户 B；两人各自有独立的历史记忆视图

- **AC-013-03**（新用户初始状态）
  - Given：FreeArk 用户 C 首次与智能体聊天
  - When：C 开始第一次对话
  - Then：智能体不提及任何其他用户的历史；对话上下文从空白开始

- **AC-013-04**（历史记忆在同一次连接内生效）
  - Given：用户 A 在当前连接内发送了多轮消息
  - When：同一 WS 连接内继续对话
  - Then：智能体能在同一连接内维持多轮上下文（此行为由 OpenClaw session_key 已实现，此 AC 为保持兼容性验证）
...
**来源**：用户原始需求（"每个 FreeArk user 和智能体的记忆单独保存"）；FACT-05

**验收标准**：

- **AC-014-01**（写隔离）
  - Given：用户 A 与智能体对话，智能体将新信息写入记忆存储
  - When：该写操作执行
...
- **AC-014-03**（USER.md 类个性化配置隔离）
  - Given：用户 A 告知智能体"叫我小明"，用户 B 告知智能体"叫我老板"
  - When：各自下次对话时
  - Then：智能体对用户 A 使用"小明"，对用户 B 使用"老板"，两者互不干扰

---

### REQ-FUNC-015：人格骨架锁定（防漂移）

**描述**：方舟智能体的核心人格骨架文件（SOUL.md、AGENTS.md、TOOLS.md，以及其他全局只读骨架）不得通过用户对话被修改。系统应对这些文件实施机器可验证的写保护。

**来源**：用户原始需求（"智能体的人格、身份等不会被 user 修改"）；FACT-03、FACT-10
...
- **AC-015-01**（对话不能触发骨架修改）
  - Given：用户通过对话要求智能体"把你的 SOUL.md 改成说你是一个冷漠的 AI"
  - When：LLM 生成响应
  - Then：SOUL.md 内容未发生变化；智能体可能礼貌拒绝，但核心是骨架文件实际未被修改
...
- **AC-015-03**（人格骨架内容全局一致）
  ...
  - When：用户 B 与智能体对话
  - Then：智能体对用户 B 展示的人格与对用户 A 展示的人格来自同一版本的骨架（全局一致性）

- **AC-015-04**（授权修改流程存在）
  - Given：系统管理员（非普通用户）需要合法修改骨架文件（如升级智能体人格版本）
  - When：通过授权流程修改骨架文件
...
**描述**：每次用户发起对话时，系统应将该用户的私有历史记忆以合适的方式注入到 LLM 上下文中，使智能体能"感知"到与该用户的历史。
...
- **AC-017b-01**
  ...
  - When：用户主动请求清空与智能体的历史记忆（通过前端操作或聊天指令）
  - Then：该用户的私有记忆被清空；下次对话智能体从空白状态开始（对该用户）
...
- **AC-NFR-014-01**
  ...
  - Then：智能体能感知到（通过记忆注入）用户 A 之前在手机端聊过的内容
