...
| CR-M1-02 | INFO | `_match_panel_sub_types()` 对 `panel_fourth_children` 的「房间数量 >= 4」判断可能在极少数情况下（如四房户型未完整同步，只同步了 4 个普通房间而无儿童房名称）产生误判。但设计文档 §1.3 已明确此为 v0.5.7 的简化版本，且优先级低（实际屋侧传回的 `ori_room_name` 会包含「儿童房」关键词）。 |
