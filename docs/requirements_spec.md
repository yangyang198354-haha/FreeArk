...
### 4.6 设备实时参数卡片（非专有部分设备）

#### REQ-FUNC-033：非专有部分设备实时参数卡片面板
- 面向无传统 PLC `specific_part`（`楼栋-单元-楼层-房号`）标识的设备，例如暖通子系统的主温控器、各房间温控面板、新风机、能耗表、水力模块等
- 前端提供独立的设备卡片展示页面，按系统分组（如：暖通）
- 每个分组内按设备类型进一步细分子标题（如：主温控器 / 温控面板 / 新风 / 能耗表 / 水力模块）
- 每张设备卡片展示：
  - 卡片标题（设备名称，如"书房-温控面板"）
  - 参数键值对列表（从 `PLCLatestData` 或同等数据源取最新快照，如 NTC温度、设备状态、湿度、开关、故障等）
  - "历史数据 >" 跳转链接，指向该设备的历史参数记录
- 接口：`GET /api/devices/realtime-params/`
  - 支持 `group` 过滤（如 `?group=hvac`）
...
- 支持过滤：`param_name`、`start_time`、`end_time`
- 支持分页：`page`（默认 1）、`page_size`（默认 50）
- 按 `collected_at` 倒序返回历史快照列表
