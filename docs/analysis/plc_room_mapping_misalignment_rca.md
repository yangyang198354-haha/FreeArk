...
## -1. 生产标定实测结论（2026-08-01，本报告的最终答案）

> 以下由 `panel_calib_capture` + `panel_calib_correlate` 在生产实测得出，
> 采集 30 分钟／28838 条屏端记录／399 户，成功标定 355 户，**推翻了本报告
> 第 5 节此前基于 plc_config 标注的推论**。第 1~7 节保留为推理过程存档。

**真实映射（全小区一致，253 户 high 置信度零例外）：**

| 槽位偏移 | param 前缀 | **三房实测**（120 户） | **四房实测**（233 户） | plc_config 标注 | Web 现行标签 |
|---|---|---|---|---|---|
| 1395 | `children_room` | 儿童房 ✓ | **书房** ✗ | 三房儿童房／四房主卧 | 主卧 |
| 1455 | `bedroom` | 主卧 ✓ | 次卧 ✓ | 三房主卧／四房次卧 | 次卧 |
| 1515 | `study_room` | 次卧 ✓ | **主卧** ✗ | 三房次卧／四房书房 | 书房 |
| 1575 | `fourth_children_room` | （无） | 儿童房 ✓ | 四房儿童房 | 儿童房 |

**两条关键结论：**

1. **`plc_config.json` 的三房分支正确**（120 户实证，97 户 high），
   **四房分支错误——「主卧」与「书房」标反了**（233 户实证，156 户 high）。
   此前"plc_config 标注可作真值"的假设仅对三房成立。
2. 实测配对恰好等于「云端 deviceSn 升序 ↔ PLC 偏移量升序」
   （书房 22552→1395、次卧 22553→1455、主卧 22554→1515、儿童房 22555→1575），
   即第 5.1 节的假设 B 成立。

**当前生产错位面（Web + 小程序监控页，两端同源共错）：**

- **四房 233 户**：主卧 ↔ 书房 两个标签互换；次卧／儿童房正确。
- **三房 120 户**：三个标签全错，且凭空显示一个该户不存在的「书房」，真实的「儿童房」不显示。
- 小程序**参数设置页**不受影响（走屏端 device_sn，端到端正确）；故障管理同样不受影响。

**样本 3-1-7-702 复核**（high，margin=0.4127，四对 score 均 1.0）：
`1395→书房`（Web 显示主卧）、`1455→次卧` ✓、`1515→主卧`（Web 显示书房）、`1575→儿童房` ✓
——与用户报告的「Web 显示主卧开启、实际书房开启」逐字吻合。
