...
# 系统架构设计 — v1.9.0 多图提问（基于 v1.5.0 增量）
...
v1.9.0 以**最小侵入增量原则**在该链路上扩展多图能力（最多5张），核心约束：
...
[前端 ChatView.vue]
  ...
  │   }
  │
  [后端 consumers.py::ChatConsumer.receive — v1.9.0 扩展]
  ...
  │   
  [adapter.py::LangGraphAdapter.stream_chat — v1.9.0 扩展]
  │   upload_ids 非空时：
