...
# 模块设计文档 — v1.9.0 多图提问（基于 v1.5.0 增量）
...
```
// api.js 新增函数（v1.9.0 新增，基于 v1.5.0 uploadChatImage 复用）
...
// ChatView.vue state 变更（v1.9.0 修改）
// 旧（v1.5.0）：
...
// ChatView.vue 函数变更（v1.9.0 修改）
onImageSelect(event: Event) → void
  - v1.5.0：读取单个文件，设置 selectedImageBlob/previewDataURL
  - v1.9.0：读取 event.target.files（可多选），逐个处理：
...
```
// vision_progress 消息处理扩展（v1.9.0）
case "vision_progress":
  - v1.5.0：显示单一进度提示"正在分析图片，请稍候…"
  - v1.9.0：直接显示服务端传来的 message 文字（"正在分析第N/T张图片，请稍候…"）
  - 在收到 stream_token / stream_end / error 时自动隐藏（行为不变）

// 新增 error code 处理（v1.9.0）
...
renderUserMessage(message: {content: string, ...}) → VNode
  ...
  - v1.9.0：content 以 "[图片1描述：" / "[图片2描述：" 等多图格式开头，同样渲染「含图提问」标注
...
**新增接口契约**（v1.9.0 新增，v1.5.0 已有接口不变）：
...
```python
# receive 方法变更（v1.9.0，基于 v1.5.0 扩展）
...
# _handle_chat 方法变更（v1.9.0，基于 v1.5.0 扩展）
async def _handle_chat(
    self,
    user_message: str,
    upload_ids: list[str] | None = None,  # v1.9.0：参数名改为 upload_ids（列表），默认 None
) -> None:
    """
    v1.9.0 变更（相对 v1.5.0）：
    ...
    3. vision_progress 帧：
       ...
       v1.9.0：不在 consumers 层发送 vision_progress（改由 adapter 通过 yield 回传）
    ...
    4. adapter.stream_chat 调用变更：
       ...
       v1.9.0：adapter.stream_chat(message=..., session_key=..., upload_ids=upload_ids, user_id=...)
...
# _pump 方法变更（v1.9.0 新增 kind 处理）
async def _pump(self, agen, accumulated_prefix: str = '') -> tuple[str, str]:
    """
    v1.9.0 新增 kind 处理（在 v1.5.0 基础上追加）：

    elif kind == 'vision_progress':
        # v1.9.0 新增（ADR-MI-004 设计）：adapter 通过此 kind 传递多图进度文字
        ...
        # v1.9.0 新增（ADR-MI-004，REQ-MI-009）：
...
```python
# connect() 新增初始化（v1.9.0，在 v1.5.0 基础上无需新增 — upload_ids 为局部变量，不需要实例变量）
...
**接口变更（v1.9.0，相对 v1.5.0 增量）**：

```python
# stream_chat 签名变更（v1.9.0）
@classmethod
async def stream_chat(
    cls,
    message: str,
    session_key: str,
    upload_ids: list[str] | None = None,   # v1.9.0 新参数（替代 upload_id）
...
| `IMAGE_TOO_MANY` | **v1.9.0 新增** | upload_ids 长度 > 5 | `error` | 阻塞（return）| consumers（MOD-MI-04）|
| `IMAGE_ANALYSIS_PARTIAL` | **v1.9.0 新增** | 部分图片 VLM 失败（非全部）| `error` | **非阻塞**（stream 继续）| consumers._pump（MOD-MI-04）|

---

## 4. 模块依赖关系图（v1.9.0 增量，基于 v1.5.0）
