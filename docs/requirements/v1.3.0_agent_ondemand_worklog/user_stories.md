...
Then   存在「方舟智能体」可展开/收起的子菜单组（el-sub-menu）
  ...
  And  原独立「和方舟智能体聊天」菜单项不再单独出现（已归入分组）
```
...
### US-NAV-02：菜单文案更新（「智能体」→「智能体」）

**As a** 所有已登录用户  
**I want to** 看到「和方舟智能体聊天」（而非「和方舟智能体聊天」）  
...
When   查看子菜单第一项文案
Then   显示「和方舟智能体聊天」（不再显示「和方舟智能体聊天」）
  And  点击后路由仍跳转到 /chat
...
| US-NAV-02  | 菜单文案「智能体」→「智能体」      | P0     | REQ-FUNC-NAV-002                 | OQ-6          |
