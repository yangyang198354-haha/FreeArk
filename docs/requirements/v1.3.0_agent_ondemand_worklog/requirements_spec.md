...
- **导航结构模糊**：聊天功能入口名为"和方舟龙虾聊天"，与规划中的智能体能力群不在同一分组，不利于后续扩展。
...
**现状**: `Layout.vue` 中「和方舟龙虾聊天」为独立 `el-menu-item`（`index="/chat"`），无分组。  
**需求**: 将当前独立「和方舟龙虾聊天」`el-menu-item` 替换为 `el-sub-menu`（`index="agent"`），子菜单含：
1. 「和方舟智能体聊天」（原「和方舟龙虾聊天」）
2. 「巡检智能体工作日志」（新建页面）
...
> - **OQ-6 聊天路由** = 保持 `/chat` 不变（仅菜单归类 + 文案「和方舟龙虾聊天」→「和方舟智能体聊天」）。
