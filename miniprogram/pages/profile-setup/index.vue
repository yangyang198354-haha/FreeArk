...
<template>
  <view class="setup-page">
    ...
    <!-- 头像选择（在 scroll-view 外 + z-index:5 防遮挡） -->
...
const safeBottom = sysInfo.safeAreaInsets ? (sysInfo.safeAreaInsets.bottom || 0) : 0
...
onLoad((options) => {
  ...
  if (pageMode.value === 'edit') {
    if (authStore.nickname) nicknameValue.value = authStore.nickname
  }
})
...
async function onSave() {
  ...
  try {
    ...
    setTimeout(() => {
      if (pageMode.value === 'initial') uni.reLaunch({ url: '/pages/home/index' })
      else uni.navigateBack()
    }, 600)
  } catch (err) {
    uni.showToast({ title: err.message || '保存失败，请重试', icon: 'none', duration: 2000 })
  } finally {
    saving.value = false
  }
}
...
function goSkip() { uni.reLaunch({ url: '/pages/home/index' }) }
</script>
...
.back-btn, .skip-btn { font-size: 28rpx; color: #2ff4e0; letter-spacing: 2rpx; }
...
.corner { position: absolute; width: 44rpx; height: 44rpx; pointer-events: none; }
...
.avatar-card { padding: 44rpx; display: flex; justify-content: center; margin-bottom: 32rpx; position: relative; z-index: 5; }
...
.btn-txt { font-weight: 700; font-size: 36rpx; letter-spacing: 16rpx; color: #04121f; }
