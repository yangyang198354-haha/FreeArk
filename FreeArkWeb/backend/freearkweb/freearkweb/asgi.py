...
启动命令（部署时修改 systemd 服务文件，见 module_design.md MOD-OPS-01）：
  uvicorn freearkweb.asgi:application --host 0.0.0.0 --port 8000 --workers 2
  perf-P1a：--workers 1 → --workers 2（换用 RedisChannelLayer，见 ADR-P1A-001）。
  依赖：redis-server 须先启动（apt 安装，loopback 127.0.0.1:6379）。
