"""
test_multi_image_question.py — v1.6.0 多图提问（Multi-Image Question）扩展单元/集成测试

覆盖 AC-MI-*（v1.6.0 新增验收标准）：
...
class TestPumpNewKinds(unittest.IsolatedAsyncioTestCase):
    """ChatConsumer._pump 中 v1.6.0 新增 kind 的处理逻辑单元测试。
...
class TestConsumersMultiImageIntegration(TransactionTestCase):
    """TC-INT-MI-040 ~ TC-INT-MI-044：consumers + adapter mock WS 集成测试（v1.6.0 多图）。"""
