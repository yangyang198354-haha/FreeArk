...
def _parse_operator_user_id(operator_override: str) -> Optional[int]:
    """operator_override 可能多种形态：int/数字字符串 / user pk。

    signer_context['user_id'] 一般就是 user.pk（int 或数字 str），
    对 persona 场景足够了；若异常形态则返回 None，工具内部拒绝。"""
    if operator_override is None:
        return None
    if isinstance(operator_override, int):
        return operator_override
    s = str(operator_override).strip()
...
def execute_write(tool_name: str, args: dict, operator_override: str) -> dict:
    """gate 节点批准后真执行写操作。分三类：
      ...
      2. 进程内自写工具（set_persona / 未来的 USER_SELF 写工具）：
         不走 HTTP，把 operator_override 映射为 user_id，直接调用 @tool
         本体（以 _user_id=signer_user_id 注入 args），等价于 owner 自己写
         自己账号的设置（安全前提：OWNER_SELF_TOOLS 在 ScopeEnforcer 侧
         已拒绝 admin/operator 调；gate.verify_write_scope 对 set_persona
         跳过 specific_part 校验——它没有 specific_part 字段）。
      3. 未知工具：返回失败信封。
    ...
    if handler_name.startswith("__INLINE_"):
        ...
        uid = _parse_operator_user_id(operator_override)
        inline_args["_user_id"] = uid
...
def _poll_write_status_until_final(
    ...
    *,
    ...
    bound_set: Optional[frozenset] = (
        frozenset(bound_specific_parts) if bound_specific_parts else None
    )
