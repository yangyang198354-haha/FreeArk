...
class LangGraphAdapter:
    ...
    async def stream_chat(
        ...
        # v1.13.0：人格指令文本（调用方经 api.persona.build_persona_instruction 构造）。
        # 本层只透传不解释——人格是领域策略，编排层与适配层都不该懂它的 schema。
        ...
        try:
            ...
            # v1.12.0（MOD-P1204）：活跃房间注入初始 State
            if active_specific_part is not None:
                _payload["active_specific_part"] = active_specific_part
            # v1.13.0：人格指令文本（不透传则本轮不注入人格块）
            if persona_prompt:
                _payload["persona_prompt"] = persona_prompt
