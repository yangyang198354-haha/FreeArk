...
class ChatConsumer(AsyncWebsocketConsumer):
    ...
    async def receive(self, text_data=None, bytes_data=None):
        ...
        # v1.9.0：多字段解析（ADR-MI-003 优先规则）
        # 新字段 image_upload_ids（列表）优先；旧字段 image_upload_id（字符串）向后兼容
        image_upload_ids_raw = data.get('image_upload_ids')   # v1.9.0 新字段（列表）
        ...
        if upload_ids and not user_message:
            if len(upload_ids) > 1:
                user_message = "请帮我分析这些图片"   # v1.9.0 多图默认文案
            else:
    ...
    async def _handle_chat(self, user_message: str, upload_ids=None):
        """
        核心聊天处理（v1.9.0）：在 v1.5.0 基础上扩展为多图支持。

        v1.9.0 变更（相对 v1.5.0，MOD-MI-04）：
        ...
        try:
            # ── v1.9.0：逐图 TTL 预检（upload_ids 存在时，逐图校验）
            ...
            # v1.3 / v1.9.0：写入用户消息记录（REQ-FUNC-007，MOD-MQ-04/MI-04）
            # 向后兼容策略：
            ...
            # v1.9.0：vision_progress 帧改由 adapter 通过 kind='vision_progress' yield，
    ...
    async def _pump(self, agen, accumulated_prefix: str = ''):
        ...
        async for kind, text in agen:
            if kind == 'reasoning':
                ...
                # v1.9.0 新增（MOD-MI-04，ADR-MI-004）：
...
class MiniAppChatConsumer(ChatConsumer):
    ...
    async def _handle_chat(self, user_message: str, upload_ids=None):
        ...
        注意：upload_ids（图文混合）路径对小程序业主当前不开放，stream_chat 调用固定传
